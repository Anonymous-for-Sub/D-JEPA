...
def reference_check(run, data_root, split, report):
    ...
    # Public configurations use the unified paper name. Released authorities
    # may retain historical keys so their signed contents remain immutable.
    if method == 'd_jepa':
        candidates = ('d_jepa', 'dtail_jepa_r', 'da_jepa_r')
        container = authority['methods'] if reference['kind'] == 'selected_ids' else authority['media_rows'][0]['methods']
        method = next((candidate for candidate in candidates if candidate in container), None)
        if method is None:
            raise ValueError('D-JEPA entry is absent from the reference authority')
